// Test script to verify updated issue extraction from page.html
const fs = require('fs');
...
// Copy the updated extractIssues function from popup.js
...
// Run the test
console.log('Testing updated issue extraction...\n');
const result = extractIssues();
const issues = result.issues || result;
const debugInfo = result.debugInfo || {};

console.log('\n=== Page Info ===');
console.log('URL:', debugInfo.pageUrl);
console.log('Has Turbo Frame:', debugInfo.hasTurboFrame);
console.log('Has React App:', debugInfo.hasReactApp);
console.log('Has React Partial:', debugInfo.hasReactPartial);
...
if (issues.length === 25) {
  console.log('\n✓ TEST PASSED: Extracted all 25 issues!');
  process.exit(0);
} else {
  console.log('\n✗ TEST FAILED: Expected 25 issues, got', issues.length);
  process.exit(1);
}
